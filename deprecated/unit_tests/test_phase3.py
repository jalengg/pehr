...
def test_config():
    ...
    assert config.data.num_patients == 10000  # Updated: increased from 3000
    assert config.model.n_num_features == 1
    assert len(config.model.cat_cardinalities) == 1  # Updated: race removed, only gender
...
def test_dataset_and_collator():
    ...
    assert batch['x_cat'].shape == (batch_size, 1)  # Changed: race removed, only gender
...
def test_training_step():
    ...
    model = PromptBartModel(
        config=bart_config,
        n_num_features=1,
        cat_cardinalities=[2],  # Updated: race removed, only gender
        prompt_length=1
    )
