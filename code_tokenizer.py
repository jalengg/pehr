...
class DiagnosisCodeTokenizer:
    ...
    def encode_codes(self, codes: List[str]) -> List[int]:
        ...
        Handles both special tokens (e.g., '<mask>') and regular diagnosis codes.

        Args:
            codes: List of diagnosis code strings.

        Returns:
            List of token IDs (offset by code_offset for regular codes).
        """
        token_ids = []
        for code in codes:
            # Check if it's a special token first
            if code in self.special_token_ids:
                token_ids.append(self.special_token_ids[code])
            else:
                # Regular diagnosis code
                vocab_idx = self.vocab.code2idx[code]
                token_ids.append(vocab_idx + self.code_offset)
        return token_ids
